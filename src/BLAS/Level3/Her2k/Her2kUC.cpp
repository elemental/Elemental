...
template<typename T>
void
Elemental::BLAS::Internal::Her2kUC
...
template<typename T>
void
Elemental::BLAS::Internal::Her2kUC_Update
...
{
#ifndef RELEASE
    PushCallStack("BLAS::Internal::Her2kUC_Update");
#endif
        ...
        B_Star_MC.Width()  != B_Star_MR.Width()     )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Nonconformal Her2kUC_Update: " <<
        ...
        B_Star_MR.RowAlignment() != C.RowAlignment()    )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Misaligned Her2kUC_Update: " <<
...
template void Elemental::BLAS::Internal::Her2kUC
( const float alpha, const DistMatrix<float,MC,MR>& A,
                     const DistMatrix<float,MC,MR>& B,
  const float beta,        DistMatrix<float,MC,MR>& C );

template void Elemental::BLAS::Internal::Her2kUC_Update
