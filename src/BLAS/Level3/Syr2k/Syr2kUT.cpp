...
template<typename T>
void
Elemental::BLAS::Internal::Syr2kUT_Update
...
{
#ifndef RELEASE
    PushCallStack("BLAS::Internal::SyrkUT_Update");
#endif
        ...
        B_Star_MC.Width()  != B_Star_MR.Width()     )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Nonconformal Syr2kUT_Update: " <<
        ...
        B_Star_MR.RowAlignment() != C.RowAlignment()    )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Misaligned Syr2kUT_Update: " <<
...
template void Elemental::BLAS::Internal::Syr2kUT_Update
