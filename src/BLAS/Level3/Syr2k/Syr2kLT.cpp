...
template<typename T>
void
Elemental::BLAS::Internal::Syr2kLTUpdate
  ...
  const T beta,        DistMatrix<T,MC,  MR>& C         )
{
        ...
        B_Star_MC.Width()  != B_Star_MR.Width()     )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Nonconformal Syr2kLTUpdate: " <<
...
template void Elemental::BLAS::Internal::Syr2kLTUpdate
