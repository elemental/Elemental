...
template<typename T>
void
Elemental::BLAS::Internal::Syr2kLT
...
template<typename T>
void
Elemental::BLAS::Internal::Syr2kLT_Update
...
{
#ifndef RELEASE
    PushCallStack("BLAS::Internal::SyrkLT_Update");
#endif
        ...
        B_Star_MC.Width()  != B_Star_MR.Width()     )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Nonconformal Syr2kLT_Update: " <<
        ...
        B_Star_MR.RowAlignment() != C.RowAlignment()    )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Misaligned Syr2kLT_Update: " <<
...
template void Elemental::BLAS::Internal::Syr2kLT
( const float alpha, const DistMatrix<float,MC,MR>& A,
                     const DistMatrix<float,MC,MR>& B,
  const float beta,        DistMatrix<float,MC,MR>& C );

template void Elemental::BLAS::Internal::Syr2kLT_Update
