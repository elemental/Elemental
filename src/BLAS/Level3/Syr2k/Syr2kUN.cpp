...
template<typename T>
void
Elemental::BLAS::Internal::Syr2kUN_Update
...
{
#ifndef RELEASE
    PushCallStack("BLAS::Internal::Syr2kUN_Update");
#endif
        ...
        B_MC_Star.Width()  != B_MR_Star.Width()    )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Nonconformal Syr2kUN_Update: " <<
        ...
        B_MR_Star.ColAlignment() != C.RowAlignment()    )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Misaligned Syr2kUN_Update: " << 
...
template void Elemental::BLAS::Internal::Syr2kUN_Update
