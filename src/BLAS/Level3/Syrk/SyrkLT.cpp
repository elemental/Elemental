  ...
  const T beta,        DistMatrix<T,MC,  MR>& C         )
{
        ...
        A_Star_MC.Width()  != A_Star_MR.Width()    )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Nonconformal SyrkLTUpdate: " <<
    ...
    {
        BLAS::Internal::SyrkLTUpdateKernel
        ( alpha, A_Star_MC, A_Star_MR, beta, C );
    }
    else
    {
        // Split C into four roughly equal pieces, perform a large gemm on CBL
...
template<typename T>
void
Elemental::BLAS::Internal::SyrkLTUpdateKernel
( const T alpha, const DistMatrix<T,Star,MC>& A_Star_MC,
                 const DistMatrix<T,Star,MR>& A_Star_MR,
  const T beta,        DistMatrix<T,MC,  MR>& C         )
{
#ifndef RELEASE
    PushCallStack("BLAS::Internal::SyrkLTUpdateKernel");
#endif
    const Grid& grid = C.GetGrid();
#ifndef RELEASE
    if( A_Star_MC.GetGrid() != A_Star_MR.GetGrid() || 
        ...
        A_Star_MC.Width()  != A_Star_MR.Width()    )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Nonconformal SyrkLTUpdateKernel: " <<
            endl << "  A[* ,MC] ~ " << A_Star_MC.Height() << " x "
                                    << A_Star_MC.Width()  <<
            endl << "  A[* ,MR] ~ " << A_Star_MR.Height() << " x "
                                    << A_Star_MR.Width()  <<
        ...
        A_Star_MR.RowAlignment() != C.RowAlignment()   )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Misaligned SyrkLTUpdateKernel: " <<
            endl << "  A[* ,MC] ~ " << A_Star_MC.RowAlignment() <<
            endl << "  A[* ,MR] ~ " << A_Star_MR.RowAlignment() <<
            endl << "  C[MC,MR] ~ " << C.ColAlignment() << " , " <<
                                       C.RowAlignment() << endl;
