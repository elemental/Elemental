  ...
  const T beta,        DistMatrix<T,MC,MR  >& C         )
{
        ...
        A_MC_Star.Width()  != A_MR_Star.Width()    )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Nonconformal SyrkUN_Update: " <<
...
template void Elemental::BLAS::Internal::SyrkUN_Update
