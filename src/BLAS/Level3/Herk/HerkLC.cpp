  ...
  const T beta,        DistMatrix<T,MC,  MR>& C         )
{
        ...
        A_Star_MC.Width()  != A_Star_MR.Width()    )
    {
        if( grid.VCRank() == 0 )
        {
            cerr << "Nonconformal HerkLC_Update: " <<
...
template void Elemental::BLAS::Internal::HerkLC_Update
