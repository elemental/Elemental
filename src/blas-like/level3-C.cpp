...
#define C_PROTO_NOINT(SIG,SIGBASE,T) \
  ...
  { EL_TRY( \
      MultiShiftQuasiTrsm( \
        ...
        DM_VR_STAR_CAST_CONST(T,shifts), DM_CAST(T,B) ) ) } \
  /* MultiShiftTrsm */ \
  ...
  ( ElLeftOrRight side, ElUpperOrLower uplo, ElOrientation orientation, \
    CREFLECT(T) alpha, ElConstMatrix_ ## SIG A, ElMatrix_ ## SIG B ) \
  { EL_TRY( \
      QuasiTrsm( \
        Reinterpret(side), Reinterpret(uplo), Reinterpret(orientation), \
        Reinterpret(alpha), *Reinterpret(A), *Reinterpret(B) ) ) } \
  ...
  ( ElLeftOrRight side, ElUpperOrLower uplo, ElOrientation orientation, \
    CREFLECT(T) alpha, ElConstDistMatrix_ ## SIG A, ElDistMatrix_ ## SIG B ) \
  { EL_TRY( \
      QuasiTrsm( \
        Reinterpret(side), Reinterpret(uplo), Reinterpret(orientation), \
        Reinterpret(alpha), DM_CAST_CONST(T,A), DM_CAST(T,B) ) ) } \
  /* Symm */ \
  ElError ElSymm_ ## SIG \
  ...
  { EL_TRY( Trdtrmm( Reinterpret(uplo), DM_CAST(T,A), conjugate ) ) } \
  /* TrdtrmmQuasi */ \
  ...
  { EL_TRY( \
      Trdtrmm( \
        Reinterpret(uplo), DM_CAST(T,A), DM_MD_STAR_CAST_CONST(T,dOff), \
        conjugate ) ) } \
  /* Trmm */ \
  ElError ElTrmm_ ## SIG \
  ( ElLeftOrRight side, ElUpperOrLower uplo, \
  ...
  ( ElUpperOrLower uplo, ElDistMatrix_ ## SIG A, bool conjugate ) \
  { EL_TRY( Trtrmm( Reinterpret(uplo), DM_CAST(T,A), conjugate ) ) } \
  /* TwoSidedTrmm */ \
