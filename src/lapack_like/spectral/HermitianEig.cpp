  ...
  const HermitianEigCtrl<F>& ctrl )
{
    ...
    return herm_eig::BlackBox( uplo, A, w, Q, ctrl );
}
  ...
  const HermitianEigCtrl<F>& ctrl )
{
    ...
    if( !IsDistributed(APre) && !IsDistributed(w) && !IsDistributed(QPre) )
    {
    ...
    return herm_eig::MRRR( uplo, APre, w, QPre, ctrl );
}
