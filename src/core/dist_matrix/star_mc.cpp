...
DistMatrix<T,STAR,MC,Int>::SumOverRow()
{
    ...
    {
        const T* thisCol = &thisBuffer[jLoc*thisLDim];
...
{ 
    ...
    if( this->RowAlignment() == A.ColAlignment() % g.Height() )
    {
        ...
        if( conjugate )
        {
            ...
            {
                T* destCol = &originalData[jLoc*height];
        ...
        for( Int k=0; k<c; ++k )
        {
            ...
            for( Int jLoc=0; jLoc<localWidth; ++jLoc )
            {
                ...
                T* thisCol = &thisBuffer[(rowOffset+jLoc*c)*thisLDim];
    ...
    else
    {
        ...
        if( conjugate )
        {
            ...
            for( Int jLoc=0; jLoc<localHeightOfA; ++jLoc )
            {
                T* destCol = &secondBuffer[jLoc*height];
                const T* sourceCol = &ABuffer[jLoc];
                for( Int i=0; i<height; ++i )
                    destCol[i] = Conj( sourceCol[i*ALDim] );
            }
    ...
    if( A.Height() == 1 )
    {
        ...
        // A[* ,VR] <- A[* ,MR]
        {
            ...
            for( Int jLoc=0; jLoc<thisLocalWidth; ++jLoc )
                sendBuf[jLoc] = ABuffer[(offset+jLoc*r)*ALDim];
        }
...
{ 
    ...
    if( this->RowAlignment() == A.RowAlignment() )
    {
        ...
        for( Int jLoc=0; jLoc<localWidth; ++jLoc )
        {
            ...
            T* originalDataCol = &originalData[jLoc*localHeightOfA];
        ...
        for( Int k=0; k<c; ++k )
        {
            ...
            for( Int jLoc=0; jLoc<localWidth; ++jLoc )
            {
                T* destCol = &thisBuffer[colShift+jLoc*thisLDim];
    ...
    else
    {
        ...
        {
            const T* ACol = &ABuffer[jLoc*ALDim];
            T* secondBufferCol = &secondBuffer[jLoc*localHeightOfA];
...
{ 
    ...
    if( this->RowAlignment() == A.RowAlignment() % g.Height() )
    {
        ...
        {
            const T* ACol = &ABuffer[jLoc*ALDim];
            T* originalDataCol = &originalData[jLoc*height];
    ...
    else
    {
        ...
        {
            const T* ACol = &ABuffer[jLoc*ALDim];
            T* secondBufferCol = &secondBuffer[jLoc*height];
...
template DistMatrix<int,STAR,MC,int>::DistMatrix( const DistMatrix<int,CIRC,CIRC,int>& A );
...
template DistMatrix<float,STAR,MC,int>::DistMatrix( const DistMatrix<float,CIRC,CIRC,int>& A );
...
template DistMatrix<double,STAR,MC,int>::DistMatrix( const DistMatrix<double,CIRC,CIRC,int>& A );
...
template DistMatrix<Complex<float>,STAR,MC,int>::DistMatrix( const DistMatrix<Complex<float>,CIRC,CIRC,int>& A );
...
template DistMatrix<Complex<double>,STAR,MC,int>::DistMatrix( const DistMatrix<Complex<double>,CIRC,CIRC,int>& A );
