...
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::AlignWithDiagonal"))
    this->SetGrid( *data.grid );
    if( (data.colDist == MR   && data.rowDist == STAR) ||
        (data.colDist == STAR && data.rowDist == MR  ) )
    {
        const Int alignment = ( data.colDist==MR ? data.colAlign
                                                 : data.rowAlign );
        if( offset >= 0 )
...
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::AlignedWithDiagonal"))
...
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::GetDiagonal"))
...
template<typename T>
void
DM<T>::GetDiagonal( DistMatrix<T,STAR,MR>& d, Int offset ) const
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::GetDiagonal"))
    this->GetDiagonalHelper
    ( d, offset, []( T& alpha, T beta ) { alpha = beta; } );
...
template<typename T>
void
DM<T>::GetRealPartOfDiagonal( DistMatrix<Base<T>,MR,STAR>& d, Int offset ) const
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::GetRealPartOfDiagonal"))
    this->GetDiagonalHelper
...
template<typename T>
void
DM<T>::GetRealPartOfDiagonal( DistMatrix<Base<T>,STAR,MR>& d, Int offset ) const
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::GetRealPartOfDiagonal"))
    this->GetDiagonalHelper
    ( d, offset, []( Base<T>& alpha, T beta ) { alpha = RealPart(beta); } );
...
template<typename T>
void
DM<T>::GetImagPartOfDiagonal( DistMatrix<Base<T>,MR,STAR>& d, Int offset ) const
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::GetImagPartOfDiagonal"))
    this->GetDiagonalHelper
...
template<typename T>
void
DM<T>::GetImagPartOfDiagonal( DistMatrix<Base<T>,STAR,MR>& d, Int offset ) const
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::GetImagPartOfDiagonal"))
    this->GetDiagonalHelper
    ( d, offset, []( Base<T>& alpha, T beta ) { alpha = ImagPart(beta); } );
...
template<typename T>
DistMatrix<Base<T>,MR,STAR>
DM<T>::GetRealPartOfDiagonal( Int offset ) const
{
    DistMatrix<Base<T>,MR,STAR> d( this->Grid() );
...
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::SetDiagonal"))
...
template<typename T>
void
DM<T>::SetDiagonal( const DistMatrix<T,STAR,MR>& d, Int offset )
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::SetDiagonal"))
    this->SetDiagonalHelper
    ( d, offset, []( T& alpha, T beta ) { alpha = beta; } );
...
template<typename T>
void
DM<T>::SetRealPartOfDiagonal( const DistMatrix<Base<T>,MR,STAR>& d, Int offset )
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::SetRealPartOfDiagonal"))
    this->SetDiagonalHelper
    ( d, offset,
...
template<typename T>
void
DM<T>::SetRealPartOfDiagonal( const DistMatrix<Base<T>,STAR,MR>& d, Int offset )
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::SetRealPartOfDiagonal"))
    this->SetDiagonalHelper
    ( d, offset,
...
template<typename T>
void
DM<T>::SetImagPartOfDiagonal( const DistMatrix<Base<T>,MR,STAR>& d, Int offset )
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::SetImagPartOfDiagonal"))
    this->ComplainIfReal();
    this->SetDiagonalHelper
    ( d, offset,
...
template<typename T>
void
DM<T>::SetImagPartOfDiagonal( const DistMatrix<Base<T>,STAR,MR>& d, Int offset )
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::SetImagPartOfDiagonal"))
    this->ComplainIfReal();
    this->SetDiagonalHelper
...
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::UpdateDiagonal"))
    this->SetDiagonalHelper
...
template<typename T>
void
DM<T>::UpdateDiagonal( T gamma, const DistMatrix<T,STAR,MR>& d, Int offset )
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::SetDiagonal"))
    this->SetDiagonalHelper
    ( d, offset, [gamma]( T& alpha, T beta ) { alpha += gamma*beta; } );
...
( Base<T> gamma, const DistMatrix<Base<T>,MR,STAR>& d, Int offset )
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::UpdateRealPartOfDiagonal"))
    this->SetDiagonalHelper
    ( d, offset,
...
void
DM<T>::UpdateRealPartOfDiagonal
( Base<T> gamma, const DistMatrix<Base<T>,STAR,MR>& d, Int offset )
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::UpdateRealPartOfDiagonal"))
    this->SetDiagonalHelper
    ( d, offset,
...
( Base<T> gamma, const DistMatrix<Base<T>,MR,STAR>& d, Int offset )
{
    DEBUG_ONLY(CallStackEntry cse("[MR,* ]::UpdateImagPartOfDiagonal"))
    this->ComplainIfReal();
    this->SetDiagonalHelper
    ( d, offset,
      [gamma]( T& alpha, Base<T> beta )
...
{
    DEBUG_ONLY(
        CallStackEntry cse("[MR,* ]::GetDiagonalHelper");
        if( d.Viewing() )
...
{
    DEBUG_ONLY(
        CallStackEntry cse("[MR,* ]::SetDiagonalHelper");
        this->AssertSameGrid( d.Grid() );
        if( d.Width() != 1 )
