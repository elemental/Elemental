...
( DistMatrix<T,MD,STAR,Int>& d, Int offset ) const
{
    ...
    for( Int k=0; k<localDiagLength; ++k )
    {
        ...
        dBuffer[k] = buffer[iLoc+jLoc*ldim];
    }
}
...
( DistMatrix<T,STAR,MD,Int>& d, Int offset ) const
{
    ...
    for( Int k=0; k<localDiagLength; ++k )
    {
        const Int iLoc = iLocStart + k*(lcm/colStride);
        const Int jLoc = jLocStart + k*(lcm/rowStride);
        dBuffer[k*dLDim] = buffer[iLoc+jLoc*ldim];
    }
}
...
( const DistMatrix<T,MD,STAR,Int>& d, Int offset )
{
    ...
    for( Int k=0; k<localDiagLength; ++k )
    {
        const Int iLoc = iLocStart + k*(lcm/colStride);
        const Int jLoc = jLocStart + k*(lcm/rowStride);
        buffer[iLoc+jLoc*ldim] = dBuffer[k];
    }
}
...
( const DistMatrix<T,STAR,MD,Int>& d, Int offset )
{
    ...
    for( Int k=0; k<localDiagLength; ++k )
    {
        const Int iLoc = iLocStart + k*(lcm/colStride);
        const Int jLoc = jLocStart + k*(lcm/rowStride);
        buffer[iLoc+jLoc*ldim] = dBuffer[k*dLDim];
    }
}
...
( const DistMatrix<T,STAR,MC,Int>& A, bool conjugate )
{
    ...
    else
    {
        ...
        if( conjugate )
        {
            ...
            for( Int jLoc=0; jLoc<localWidth; ++jLoc )
            {
                T* destCol = &sendBuffer[jLoc*localWidth];
                const T* sourceCol = &ABuffer[rowShift+jLoc*rowStride];
        ...
        for( Int jLoc=0; jLoc<localWidth; ++jLoc )
            MemCopy
            ( &buffer[jLoc*ldim], &recvBuffer[jLoc*localHeight], localHeight );
    ...
    if( this->Grid() == A.Grid() )
    {
        ...
        else
        {
            ...
            for( Int jLoc=0; jLoc<localWidth; ++jLoc )
                MemCopy
                ( &buffer[jLoc*ldim], 
                  &recvBuffer[jLoc*localHeight], localHeight );
            this->auxMemory_.Release();
        }
    }
                ...
                if( inAGrid )
                {
                    ...
                    for( Int jLoc=0; jLoc<sendWidth; ++jLoc )
                    {
                        ...
                        for( Int iLoc=0; iLoc<sendHeight; ++iLoc )
                        {
                            ...
                            sendBuffer[iLoc+jLoc*sendHeight] = ABuffer[i+j*ALDim];
                        }
                    }
                    // Send data
                        ...
                        for( Int rowRecv=0; rowRecv<numRowRecvs; ++rowRecv )
                        {
                            ...
                            for( Int jLoc=0; jLoc<sendWidth; ++jLoc )
                            {
                                ...
                                for( Int iLoc=0; iLoc<sendHeight; ++iLoc )
                                {
                                    ...
                                    buffer[i+j*ldim] = recvBuffer[iLoc+jLoc*sendHeight];
                                }
                            }
    ...
    if( this->ColAlignment() == A.ColAlignment() )
    {
        if( this->Width() == 1 )
        {
            ...
            if( myCol == rowAlignment )
                MemCopy( this->Buffer(), recvBuffer, localHeight );
            this->auxMemory_.Release();
        }
        else
        {
    ...
    else
    {
        ...
        if( this->Width() == 1 )
        {
            ...
            if( myCol == rowAlignment )
            {
                ...
                MemCopy( this->Buffer(), sendBuffer, localHeight );
            }
            this->auxMemory_.Release();
        }
        else
        {
            ...
            for( Int jLoc=0; jLoc<localWidth; ++jLoc )
                MemCopy
                ( &thisBuffer[jLoc*thisLDim], 
                  &secondBuffer[jLoc*localHeight], localHeight );
            this->auxMemory_.Release();
        }
    }
}
...
template DistMatrix<int,MC,MR,int>::DistMatrix( const DistMatrix<int,CIRC,CIRC,int>& A );
...
template DistMatrix<float,MC,MR,int>::DistMatrix( const DistMatrix<float,CIRC,CIRC,int>& A );
...
template DistMatrix<double,MC,MR,int>::DistMatrix( const DistMatrix<double,CIRC,CIRC,int>& A );
...
template DistMatrix<Complex<float>,MC,MR,int>::DistMatrix( const DistMatrix<Complex<float>,CIRC,CIRC,int>& A );
...
template DistMatrix<Complex<double>,MC,MR,int>::DistMatrix( const DistMatrix<Complex<double>,CIRC,CIRC,int>& A );
