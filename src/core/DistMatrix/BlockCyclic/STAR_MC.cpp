...
BDM& BDM::operator=( const DistMatrix<T,MC,STAR,BLOCK_CYCLIC>& A )
{
    ...
    auto A_STAR_VR = MakeUnique<DistMatrix<T,STAR,VR,BLOCK_CYCLIC>>( *A_MC_MR );
    A_MC_MR.reset();
...
{ 
    ...
    auto A_MR_MC = MakeUnique<DistMatrix<T,MR,MC,BLOCK_CYCLIC>>( this->Grid() );
...
{ 
    ...
    DistMatrix<T,STAR,VC,BLOCK_CYCLIC> A_STAR_VC(this->Grid());
    A_STAR_VC.AlignRowsWith(*this);
    *this = A_STAR_VC = A;
    return *this;
}
...
BDM& BDM::operator=( const DistMatrix<T,CIRC,CIRC,BLOCK_CYCLIC>& A )
{
    ...
    DistMatrix<T,MR,MC,BLOCK_CYCLIC> A_MR_MC( A.Grid() );
