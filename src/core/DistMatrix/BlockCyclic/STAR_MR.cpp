...
BDM& BDM::operator=( const DistMatrix<T,MC,STAR,BLOCK_CYCLIC>& A )
{
    ...
    DistMatrix<T,MC,MR,BLOCK_CYCLIC> A_MC_MR(this->Grid());
...
{ 
    ...
    auto A_STAR_VC = MakeUnique<DistMatrix<T,STAR,VC,BLOCK_CYCLIC>>( A );
    auto A_STAR_VR =
      MakeUnique<DistMatrix<T,STAR,VR,BLOCK_CYCLIC>>( this->Grid() );
...
{ 
    ...
    auto A_VR_STAR = MakeUnique<DistMatrix<T,VR,STAR,BLOCK_CYCLIC>>( A );
    auto A_VC_STAR =
      MakeUnique<DistMatrix<T,VC,STAR,BLOCK_CYCLIC>>( *A_VR_STAR );
    A_VR_STAR.reset(); 

    auto A_MC_MR = MakeUnique<DistMatrix<T,MC,MR,BLOCK_CYCLIC>>( this->Grid() );
    A_MC_MR->AlignRowsWith(*this);
    *A_MC_MR = *A_VC_STAR;
    A_VC_STAR.reset();
...
{ 
    ...
    *A_STAR_VR = *A_STAR_VC;
    ...
    auto A_MC_MR = MakeUnique<DistMatrix<T,MC,MR,BLOCK_CYCLIC>>( *A_STAR_VR );
    A_STAR_VR.reset();
...
{ 
    ...
    DistMatrix<T,STAR,VR,BLOCK_CYCLIC> A_STAR_VR(this->Grid());
    A_STAR_VR.AlignRowsWith(*this);
    A_STAR_VR = A;
    *this = A_STAR_VR;
    return *this;
