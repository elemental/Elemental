  ...
  DistMatrix<T,MC,MR>& AR )
{
    ...
    {
        if( _grid->VCRank() == 0 )
            cerr << "1x2 is misaligned, cannot combine." << endl;
        DumpCallStack();
        throw exception();
    }
#endif
  ...
  DistMatrix<T,MC,MR>& ABR )
{
    ...
    {
        if( _grid->VCRank() == 0 )
            cerr << "2x2 set of matrices must align to combine." << endl;
        DumpCallStack();
        throw exception();
    }
#endif
...
( DistMatrix<T,MD,Star>& d, const int offset )
{
    ...
    {
        if( _grid->VCRank() == 0 )
            cerr << "d is not of the correct length." << endl;
        DumpCallStack();
        throw exception();
    }
#endif
...
( const DistMatrix<T,MD,Star>& d, const int offset )
{
    ...
    {
        if( _grid->VCRank() == 0 ) 
            cerr << "d must be a column vector." << endl;
        DumpCallStack();
        throw exception();
    }
    {
        ...
        if( length != d.Height() )
        {
            if( _grid->VCRank() == 0 )
            {
                cerr << "d is not of the same length as the diagonal:" << 
                endl << "  A ~ " << Height() << " x " << Width() << 
                endl << "  d ~ " << d.Height() << " x " << d.Width() <<
                endl << "  A diag length: " << length << endl;
            }
            DumpCallStack();
            throw exception();
        }
...
( const DistMatrix<T,Star,MD>& d, const int offset )
{
    ...
    {
        if( _grid->VCRank() == 0 ) 
            cerr << "d must be a row vector." << endl;
        DumpCallStack();
        throw exception();
    }
    {
