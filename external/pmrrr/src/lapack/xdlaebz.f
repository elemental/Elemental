      SUBROUTINE XDLAEBZ( IJOB, NITMAX, N, MMAX, MINP, NBMIN, ABSTOL,
...
*  =======
*
*  XDLAEBZ contains the iteration loops which compute and use the
...
*          Other values will cause XDLAEBZ to return with INFO=-1.
*
...
*          are generated, then XDLAEBZ will quit with INFO=MMAX+1.
*
*  MINP    (input) INTEGER
...
*             NAB(i,j)=N(AB(i,j)), from a previous call to XDLAEBZ with
*             IJOB=1.
...
*             distinctive value(s) before XDLAEBZ is called.
*
...
*  for two purposes:
*
*  (a) finding eigenvalues.  In this case, XDLAEBZ should have one or
*      more initial intervals set up in AB, and XDLAEBZ should be called
*      with IJOB=1.  This sets up NAB, and also counts the eigenvalues.
*      Intervals with no eigenvalues would usually be thrown out at
*      this point.  Also, if not all the eigenvalues in an interval i
*      are desired, NAB(i,1) can be increased or NAB(i,2) decreased.
*      For example, set NAB(i,1)=NAB(i,2)-1 to get the largest
*      eigenvalue.  XDLAEBZ is then called with IJOB=2 and MMAX
...
*      interval does not lie in (a,b).  XDLAEBZ is then called with
