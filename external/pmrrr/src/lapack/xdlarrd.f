      SUBROUTINE XDLARRD( RANGE, ORDER, N, VL, VU, IL, IU, GERS,
...
*  =======
*
*  XDLARRD computes the eigenvalues of a symmetric tridiagonal
...
*          eigenvalue approximations. XDLARRD computes an interval
...
*          belongs.  (XDLARRD may use the remaining N-M elements as
*          workspace.)
*
...
*     .. External Functions ..
      ...
      EXTERNAL           XLSAME, XILAENV, XDLAMCH
*     ..
...
*
      IF( IRANGE.LE.0 ) THEN
         INFO = -1
      ELSE IF( .NOT.(XLSAME(ORDER,'B').OR.XLSAME(ORDER,'E')) ) THEN
         INFO = -2
      ELSE IF( N.LT.0 ) THEN
         INFO = -3
...
*     If ORDER='E', sort the eigenvalues from smallest to largest
      ...
      IF( XLSAME(ORDER,'E') .AND. NSPLIT.GT.1 ) THEN
         DO 150 JE = 1, M - 1
            IE = 0
            TMP1 = W( JE )
            DO 140 J = JE + 1, M
