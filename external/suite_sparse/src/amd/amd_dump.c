...
GLOBAL Int ElAMD_debug = -999 ;		/* default is no debug printing */
...
GLOBAL void ElAMD_debug_init ( char *s )
{
    ...
    {
	fscanf (f, ID, &ElAMD_debug) ;
	fclose (f) ;
    }
    if (ElAMD_debug >= 0)
    {
	printf ("%s: ElAMD_debug_init, D= "ID"\n", s, ElAMD_debug) ;
    }
}
...
)
{
	...
	if (elen >= EMPTY)
	{
		...
		if (pe == EMPTY)
		{
		    EL_AMD_DEBUG3 ((" dense node\n")) ;
		    ASSERT (w == 1) ;
		}
		else
		{
		    ASSERT (pe < EMPTY) ;
		    EL_AMD_DEBUG3 ((" i "ID" -> parent "ID"\n", i, FLIP (Pe[i])));
		}
	    }
	    else
	    {
		EL_AMD_DEBUG3 (("\nI "ID": active principal supervariable:\n",i));
		EL_AMD_DEBUG3 (("   nv(i): "ID"  Flag: %d\n", nv, (nv < 0))) ;
		ASSERT (elen >= 0) ;
		ASSERT (nv > 0 && pe >= 0) ;
		p = pe ;
		EL_AMD_DEBUG3 (("   e/s: ")) ;
		if (elen == 0) EL_AMD_DEBUG3 ((" : ")) ;
		ASSERT (pe + len <= pfree) ;
		for (k = 0 ; k < len ; k++)
		{
		    j = Iw [p] ;
	...
	for (deg = 0 ; deg < n ; deg++)
	{
	    ...
	    {
		EL_AMD_DEBUG3 (("   "ID" : next "ID" last "ID" deg "ID"\n",
