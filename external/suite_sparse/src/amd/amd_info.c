...
#define PRI(format,x) { if (x >= 0) { EL_SUITESPARSE_PRINTF ((format, x)) ; }}

GLOBAL void ElAMD_info
(
    double Info [ ]
)
{
    ...
    EL_SUITESPARSE_PRINTF (("\nElAMD version %d.%d.%d, %s, results:\n",
	EL_AMD_MAIN_VERSION, EL_AMD_SUB_VERSION, EL_AMD_SUBSUB_VERSION, 
        EL_AMD_DATE)) ;
    ...
    n = Info [EL_AMD_N] ;
    ndiv = Info [EL_AMD_NDIV] ;
    nmultsubs_ldl = Info [EL_AMD_NMULTSUBS_LDL] ;
    nmultsubs_lu = Info [EL_AMD_NMULTSUBS_LU] ;
    lnz = Info [EL_AMD_LNZ] ;
    lnzd = (n >= 0 && lnz >= 0) ? (n + lnz) : (-1) ;

    /* AMD return status */
    EL_SUITESPARSE_PRINTF (("    status: ")) ;
    if (Info [EL_AMD_STATUS] == EL_AMD_OK)
    {
	EL_SUITESPARSE_PRINTF (("OK\n")) ;
    }
    else if (Info [EL_AMD_STATUS] == EL_AMD_OUT_OF_MEMORY)
    {
	EL_SUITESPARSE_PRINTF (("out of memory\n")) ;
    }
    else if (Info [EL_AMD_STATUS] == EL_AMD_INVALID)
    {
	EL_SUITESPARSE_PRINTF (("invalid matrix\n")) ;
    }
    else if (Info [EL_AMD_STATUS] == EL_AMD_OK_BUT_JUMBLED)
    {
	EL_SUITESPARSE_PRINTF (("OK, but jumbled\n")) ;
    }
    else
    {
	EL_SUITESPARSE_PRINTF (("unknown\n")) ;
    }
