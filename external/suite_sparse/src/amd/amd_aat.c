...
GLOBAL size_t ElAMD_aat	/* returns nz in A+A' */
(
    Int n,
    const Int Ap [ ],
    const Int Ai [ ],
...
)
{
...
#ifndef NDEBUG
    ElAMD_debug_init ("AMD AAT") ;
    for (k = 0 ; k < n ; k++) Tp [k] = EMPTY ;
    ASSERT (ElAMD_valid (n, n, Ap, Ai) == EL_AMD_OK) ;
#endif
	...
	for (p = p1 ; p < p2 ; )
	{
		...
		Len [j]++ ;
		Len [k]++ ;
		EL_AMD_DEBUG3 (("    upper ("ID","ID") ("ID","ID")\n", j,k, k,j));
		p++ ;
	    }
	    else if (j == k)
	    {
		...
		if (i < k)
		{
		    ...
		    Len [i]++ ;
		    Len [j]++ ;
		    EL_AMD_DEBUG3 (("    lower ("ID","ID") ("ID","ID")\n",
			i,j, j,i)) ;
		    pj++ ;
		}
		else if (i == k)
	...
	for (pj = Tp [j] ; pj < Ap [j+1] ; pj++)
	{
	    ...
	    Len [i]++ ;
	    Len [j]++ ;
	    EL_AMD_DEBUG3 (("    lower cleanup ("ID","ID") ("ID","ID")\n",
		i,j, j,i)) ;
	}
    }
    ...
    EL_AMD_DEBUG1 (("AMD nz in A+A', excluding diagonal (nzaat) = %g\n",
	(double) nzaat)) ;
    EL_AMD_DEBUG1 (("   nzboth: "ID" nz: "ID" nzdiag: "ID" symmetry: %g\n",
	...
	Info [EL_AMD_N] = n ;
	Info [EL_AMD_NZ] = nz ;
	Info [EL_AMD_SYMMETRY] = sym ;	     /* symmetry of pattern of A */
	Info [EL_AMD_NZDIAG] = nzdiag ;	     /* nonzeros on diagonal of A */
	Info [EL_AMD_NZ_A_PLUS_AT] = nzaat ; /* nonzeros in A+A' */
    }
