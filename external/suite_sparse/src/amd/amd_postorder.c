...
)
{
	...
	for (j = 0 ; j < nn ; j++)
	{
	    if (Nv [j] > 0)
	    {
		EL_AMD_DEBUG1 (( ""ID" :  nels "ID" npiv "ID" size "ID
		    " parent "ID" maxfr "ID"\n", j, nels,
		    Nv [j], Fsize [j], Parent [j], Fsize [j])) ;
		/* this is an element */
		/* dump the link list of children */
		nchild = 0 ;
		EL_AMD_DEBUG1 (("    Children: ")) ;
	...
	if (Nv [i] > 0 && Child [i] != EMPTY)
	{
...
#ifndef NDEBUG
	    Int nchild ;
	    EL_AMD_DEBUG1 (("Before partial sort, element "ID"\n", i)) ;
	    nchild = 0 ;
	    for (f = Child [i] ; f != EMPTY ; f = Sibling [f])
	    {
		ASSERT (f >= 0 && f < nn) ;
		EL_AMD_DEBUG1 (("      f: "ID"  size: "ID"\n", f, Fsize [f])) ;
		nchild++ ;
		ASSERT (nchild <= nn) ;
	    }
#endif
	    ...
	    EL_AMD_DEBUG1 (("bigf "ID" maxfrsize "ID" bigfprev "ID" fnext "ID
...
#ifndef NDEBUG
	    EL_AMD_DEBUG1 (("After partial sort, element "ID"\n", i)) ;
	    for (f = Child [i] ; f != EMPTY ; f = Sibling [f])
	    {
		ASSERT (f >= 0 && f < nn) ;
		EL_AMD_DEBUG1 (("        "ID"  "ID"\n", f, Fsize [f])) ;
	...
	{
	    EL_AMD_DEBUG1 (("Root of assembly tree "ID"\n", i)) ;
	    k = ElAMD_post_tree (i, k, Child, Sibling, Order, Stack
#ifndef NDEBUG
		, nn
#endif
		) ;
