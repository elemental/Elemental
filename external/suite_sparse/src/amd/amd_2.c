...
/* ========================================================================= */
	...
	*/
...
#ifndef NDEBUG
    EL_AMD_DEBUG1 (("\n======Nel "ID" initial\n", nel)) ;
    ElAMD_dump (n, Pe, Iw, Len, iwlen, pfree, Nv, Next, Last,
	...
	else if (deg > dense)
	{
	    ...
	    EL_AMD_DEBUG1 (("Dense node "ID" degree "ID"\n", i, deg)) ;
	    ndense++ ;
...
/* ========================================================================= */
	...
	if (ElAMD_debug >= 2)
	{
	    ElAMD_dump (n, Pe, Iw, Len, iwlen, pfree, Nv, Next,
...
/* ========================================================================= */
	...
	else
	{
		...
		if (knt1 > elenme)
		{
		    ...
		    pj = p ;
		    ln = slenme ;
		    EL_AMD_DEBUG2 (("Search sv: "ID" "ID" "ID"\n", me,pj,ln)) ;
		}
		else
		{
		    ...
		    pj = Pe [e] ;
		    ln = Len [e] ;
		    EL_AMD_DEBUG2 (("Search element e "ID" in me "ID"\n", e,me)) ;
		...
		for (knt2 = 1 ; knt2 <= ln ; knt2++)
		{
			...
			if (pfree >= iwlen)
			{
			    ...
			    EL_AMD_DEBUG1 (("GARBAGE COLLECTION\n")) ;
...
#ifndef NDEBUG
	EL_AMD_DEBUG2 (("New element structure: length= "ID"\n", pme2-pme1+1)) ;
	for (pme = pme1 ; pme <= pme2 ; pme++) EL_AMD_DEBUG3 ((" "ID"", Iw[pme]));
	EL_AMD_DEBUG3 (("\n")) ;
#endif
...
/* ========================================================================= */
	...
	for (pme = pme1 ; pme <= pme2 ; pme++)
	{
		...
		for (p = Pe [i] ; p <= Pe [i] + eln - 1 ; p++)
		{
		    ...
		    ASSERT (e >= 0 && e < n) ;
		    we = W [e] ;
		    EL_AMD_DEBUG4 (("    e "ID" we "ID" ", e, we)) ;
		    if (we >= wflg)
		    {
			/* unabsorbed element e has been seen in this loop */
			EL_AMD_DEBUG4 (("    unabsorbed, first time seen")) ;
			we -= nvi ;
		    }
		    else if (we != 0)
		    {
...
/* ========================================================================= */
	...
	for (pme = pme1 ; pme <= pme2 ; pme++)
	{
	    ...
	    EL_AMD_DEBUG2 (("Updating: i "ID" "ID" "ID"\n", i, Elen[i], Len [i]));
	    p1 = Pe [i] ;
	    p2 = p1 + Elen [i] - 1 ;
	    pn = p1 ;
	    hash = 0 ;
		...
		for (p = p1 ; p <= p2 ; p++)
		{
			...
			if (dext > 0)
			{
			    ...
			    Iw [pn++] = e ;
			    hash += e ;
			    EL_AMD_DEBUG4 ((" e: "ID" hash = "ID"\n",e,hash)) ;
			}
			else
			{
			    /* external degree of e is zero, absorb e into me*/
			    EL_AMD_DEBUG1 ((" Element "ID" =>"ID" (aggressive)\n",
				e, me)) ;
			    ASSERT (dext == 0) ;
			    Pe [e] = FLIP (me) ;
			    W [e] = 0 ;
		...
		if (nvj > 0)
		{
		    ...
		    Iw [pn++] = j ;
		    hash += j ;
		    EL_AMD_DEBUG4 (("  s: "ID" hash "ID" Nv[j]= "ID"\n",
				j, hash, nvj)) ;
		}
	    }
		...
		EL_AMD_DEBUG1 (("  MASS i "ID" => parent e "ID"\n", i, me)) ;
		Pe [i] = FLIP (me) ;
		nvi = -Nv [i] ;
		degme -= nvi ;
		nvpiv += nvi ;
...
/* ========================================================================= */
	...
	EL_AMD_DEBUG1 (("Detecting supervariables:\n")) ;
	for (pme = pme1 ; pme <= pme2 ; pme++)
	{
	    i = Iw [pme] ;
	    ASSERT (i >= 0 && i < n) ;
	    EL_AMD_DEBUG2 (("Consider i "ID" nv "ID"\n", i, Nv [i])) ;
	    if (Nv [i] < 0)
	    {
		...
		*/
		...
		while (i != EMPTY && Next [i] != EMPTY)
		{
			...
			EL_AMD_DEBUG3 (("compare i "ID" and j "ID"\n", i,j)) ;
			...
			if (ok)
			{
			    ...
			    EL_AMD_DEBUG1 (("found it! j "ID" => i "ID"\n", j,i));
			    Pe [j] = FLIP (i) ;
...
/* ========================================================================= */
	...
	EL_AMD_DEBUG2 (("restore done\n")) ;

/* ========================================================================= */
/* FINALIZE THE NEW ELEMENT */
/* ========================================================================= */

	EL_AMD_DEBUG2 (("ME = "ID" DONE\n", me)) ;
	Nv [me] = nvpiv ;
...
#ifndef NDEBUG
	EL_AMD_DEBUG2 (("finalize done nel "ID" n "ID"\n   ::::\n", nel, n)) ;
	for (pme = Pe [me] ; pme <= Pe [me] + Len [me] - 1 ; pme++)
	{
	      EL_AMD_DEBUG3 ((" "ID"", Iw [pme])) ;
	}
	EL_AMD_DEBUG3 (("\n")) ;
#endif
...
/* ========================================================================= */
	...
	Info [EL_AMD_NDIV] = ndiv ;

	/* number of multiply-subtract pairs for LDL' */
	Info [EL_AMD_NMULTSUBS_LDL] = nms_ldl ;

	/* number of multiply-subtract pairs for LU */
	Info [EL_AMD_NMULTSUBS_LU] = nms_lu ;

	/* number of "dense" rows/columns */
	Info [EL_AMD_NDENSE] = ndense ;

	/* largest front is dmax-by-dmax */
	Info [EL_AMD_DMAX] = dmax ;

	/* number of garbage collections in AMD */
	Info [EL_AMD_NCMPA] = ncmpa ;
 ...
 * is the size of element e.  Elen [i] is EMPTY for unordered variable i. */
    ...
    for (i = 0 ; i < n ; i++)
    {
	EL_AMD_DEBUG2 ((" "ID" parent: "ID"   ", i, Pe [i])) ;
	ASSERT (Pe [i] >= EMPTY && Pe [i] < n) ;
	if (Nv [i] > 0)
	{
	    /* this is an element */
	    e = i ;
	    EL_AMD_DEBUG2 ((" element, size is "ID"\n", Elen [i])) ;
	    ASSERT (Elen [e] > 0) ;
	}
	EL_AMD_DEBUG2 (("\n")) ;
    }
	...
	if (Nv [e] > 0)
	{
	    EL_AMD_DEBUG3 (("Element e= "ID" size "ID" nv "ID" \n", e,
		Elen [e], Nv [e])) ;
	}
    }
    EL_AMD_DEBUG2 (("\nvariables:\n")) ;
	...
	if (Nv [i] == 0)
	{
	    EL_AMD_DEBUG3 (("i unordered: "ID"\n", i)) ;
	    j = Pe [i] ;
	    cnt = 0 ;
	    EL_AMD_DEBUG3 (("  j: "ID"\n", j)) ;
	    if (j == EMPTY)
	    {
		EL_AMD_DEBUG3 (("	i is a dense variable\n")) ;
	    }
	    else
	    {
		ASSERT (j >= 0 && j < n) ;
...
/* ========================================================================= */
	...
	if (Nv [i] == 0)
	{
	    ...
	    EL_AMD_DEBUG1 (("Path compression, i unordered: "ID"\n", i)) ;
	    j = Pe [i] ;
	    ASSERT (j >= EMPTY && j < n) ;
	    EL_AMD_DEBUG3 (("	j: "ID"\n", j)) ;
	    if (j == EMPTY)
