...
)
{
	...
	EL_AMD_DEBUG0 (("column 0 pointer bad or nz < 0\n")) ;
	return (EL_AMD_INVALID) ;
    }
    for (j = 0 ; j < n_col ; j++)
    {
	...
	if (p1 > p2)
	{
	    ...
	    EL_AMD_DEBUG0 (("column "ID" pointer bad\n", j)) ;
	    return (EL_AMD_INVALID) ;
	}
	ilast = EMPTY ;
	for (p = p1 ; p < p2 ; p++)
	{
	    i = Ai [p] ;
	    EL_AMD_DEBUG3 (("row: "ID"\n", i)) ;
	    if (i < 0 || i >= n_row)
	    {
		/* row index out of range */
		EL_AMD_DEBUG0 (("index out of range, col "ID" row "ID"\n", j, i));
		return (EL_AMD_INVALID) ;
	    }
	    if (i <= ilast)
	    {
		/* row index unsorted, or duplicate entry present */
		EL_AMD_DEBUG1 (("index unsorted/dupl col "ID" row "ID"\n", j, i));
		result = EL_AMD_OK_BUT_JUMBLED ;
	    }
	    ilast = i ;
	}
    }
