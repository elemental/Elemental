...
*/
#ifndef ELEMENTAL_DISTMATRIX_VC_STAR_H
#define ELEMENTAL_DISTMATRIX_VC_STAR_H 1
...
{ 
    ...
    if( colAlignment < 0 || colAlignment >= grid.Size() )
    {
        if( grid.VCRank() == 0 )
        {
            std::cerr << "colAlignment for [VC,* ] must be in [0,p-1], p=rxc."
                      << std::endl;
        }
        DumpCallStack();
        throw std::exception();
...
#endif /* ELEMENTAL_DISTMATRIX_VC_STAR_H */
