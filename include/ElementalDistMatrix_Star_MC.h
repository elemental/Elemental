...
*/
#ifndef ELEMENTAL_DISTMATRIX_STAR_MC_H
#define ELEMENTAL_DISTMATRIX_STAR_MC_H 1
...
{ 
    ...
    if( rowAlignment < 0 || rowAlignment >= grid.Height() )
    {
        if( grid.VCRank() == 0 )
        {
            std::cerr << "rowAlignment for [* ,MC] must be in [0,r-1] "
...
#endif /* ELEMENTAL_DISTMATRIX_STAR_MC_H */
