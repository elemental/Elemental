...
template<typename T>
class DistMatrix<T,MC,MR,BLOCK_CYCLIC> : public BlockCyclicMatrix<T>
{
public:
    // Typedefs
    // ========
    ...
    typedef DistMatrix<T,MC,MR,BLOCK_CYCLIC> type;
    ...
    DistMatrix<T,MC,MR,BLOCK_CYCLIC>* Construct
    ( const El::Grid& g, int root ) const override;
    DistMatrix<T,MR,MC,BLOCK_CYCLIC>* ConstructTranspose
    ( const El::Grid& g, int root ) const override;
