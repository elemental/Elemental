...
// of the process grid.
template<typename T>
class DistMatrix<T,MR,MC,BLOCK_CYCLIC> : public BlockCyclicMatrix<T>
{
public:
    typedef AbstractDistMatrix<T> absType;
    typedef BlockCyclicMatrix<T> blockCyclicType;
    typedef DistMatrix<T,MR,MC,BLOCK_CYCLIC> type;
    ...
    DistMatrix<T,MR,MC,BLOCK_CYCLIC>* Construct
    ( const El::Grid& g, int root ) const override;
    DistMatrix<T,MC,MR,BLOCK_CYCLIC>* ConstructTranspose
    ( const El::Grid& g, int root ) const override;
