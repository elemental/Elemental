...
// distributed.
template<typename T>
class DistMatrix<T,STAR,VR,BLOCK_CYCLIC> : public BlockCyclicMatrix<T>
{
public:
    typedef AbstractDistMatrix<T> absType;
    typedef BlockCyclicMatrix<T> blockCyclicType;
    typedef DistMatrix<T,STAR,VR,BLOCK_CYCLIC> type;
    ...
    DistMatrix<T,STAR,VR,BLOCK_CYCLIC>* Construct
    ( const El::Grid& g, int root ) const override;
    DistMatrix<T,VR,STAR,BLOCK_CYCLIC>* ConstructTranspose
    ( const El::Grid& g, int root ) const override;
    DistMatrix<T,VR,STAR,BLOCK_CYCLIC>* ConstructDiagonal
    ( const El::Grid& g, int root ) const override;
    ...
    mpi::Comm PartialRowComm()      const override EL_NO_EXCEPT;
    mpi::Comm PartialUnionRowComm() const override EL_NO_EXCEPT;

    int ColStride()             const override EL_NO_EXCEPT;
    int RowStride()             const override EL_NO_EXCEPT;
    int PartialRowStride()      const override EL_NO_EXCEPT;
    int PartialUnionRowStride() const override EL_NO_EXCEPT;
