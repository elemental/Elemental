...
*/
#pragma once
#ifndef LAPACK_APPLYPACKEDREFLECTORS_LUVB_HPP
#define LAPACK_APPLYPACKEDREFLECTORS_LUVB_HPP
...
LUVB( int offset, const Matrix<R>& H, Matrix<R>& A )
{
    ...
    while( HBR.Height() < H.Height() && HBR.Width() < H.Width() )
    {
        ...
        SetDiagonal( HPanCopy, R(1), offset, RIGHT );

        Syrk( UPPER, TRANSPOSE, R(1), HPanCopy, SInv );
        HalveMainDiagonal( SInv );

        Gemm( TRANSPOSE, NORMAL, R(1), HPanCopy, ATop, Z );
        ...
        DistMatrix<R>& A )
{
    ...
    while( HBR.Height() < H.Height() && HBR.Width() < H.Width() )
    {
        ...
        LocalGemm( TRANSPOSE, NORMAL, R(1), HPan_MC_STAR, ATop, Z_STAR_MR );
        ...
        Matrix<Complex<R> >& A )
{
    ...
    while( HBR.Height() < H.Height() && HBR.Width() < H.Width() )
    {
        ...
        HPanCopy = HPan;
        MakeTrapezoidal( UPPER, HPanCopy, offset, RIGHT );
        SetDiagonal( HPanCopy, C(1), offset, RIGHT );

        Herk( UPPER, ADJOINT, C(1), HPanCopy, SInv );
        FixDiagonal( conjugation, t1, SInv );

        Gemm( ADJOINT, NORMAL, C(1), HPanCopy, ATop, Z );
        ...
        DistMatrix<Complex<R> >& A )
{
    ...
    while( HBR.Height() < H.Height() && HBR.Width() < H.Width() )
    {
        ...
        HPan_MC_STAR = HPanCopy;
        LocalGemm( ADJOINT, NORMAL, C(1), HPan_MC_STAR, ATop, Z_STAR_MR );
        Z_STAR_VR.SumScatterFrom( Z_STAR_MR );
...
#endif // ifndef LAPACK_APPLYPACKEDREFLECTORS_LUVB_HPP
