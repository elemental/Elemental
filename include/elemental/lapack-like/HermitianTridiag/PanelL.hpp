...
#ifndef RELEASE
    PushCallStack("internal::HermitianPanelTridiagL");
    if( A.Grid() != W.Grid() )
    ...
    while( WTL.Width() < panelSize )
    {
        ...
        if( W22.Width() > 0 )
        {
            ...
            ( &reduceToOneSendBuffer[0], 
              p21_MC_STAR.LocalBuffer(), localHeight );
            ...
            if( contributing )
            {
                if( r == c )
                {
                    ...
                    ( &reduceToOneSendBuffer[localHeight],
                      q21_MR_STAR.LocalBuffer(), localHeight );
                }
                else
                {
