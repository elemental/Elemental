      SUBROUTINE XDLARRE( RANGE, N, VL, VU, IL, IU, D, E, E2,
...
*  tridiagonal matrix T, XDLARRE sets any "small" off-diagonal
...
*  find a few eigenvalues or the dqds algorithm (subroutine XDLASQ2) to
*  conpute all and then discard any unwanted one.
*  As an added benefit, XDLARRE also outputs the n
...
*          If RANGE='I' or ='A', XDLARRE computes bounds on the desired
...
*          sorted in ascending order ( XDLARRE may use the
...
*  INFO    (output) INTEGER
*          = 0:  successful exit
*          > 0:  A problem occured in XDLARRE.
*          < 0:  One of the called subroutines signaled an internal problem.
*                Needs inspection of the corresponding parameter IINFO
*                for further information.
*
*          =-1:  Problem in XDLARRD.
*          = 2:  No base representation could be found in MAXTRY iterations.
*                Increasing MAXTRY and recompilation might be a remedy.
*          =-3:  Problem in XDLARRB when computing the refined root
*                representation for XDLASQ2.
*          =-4:  Problem in XDLARRB when preforming bisection on the
*                desired part of the spectrum.
*          =-5:  Problem in XDLASQ2.
*          =-6:  Problem in XDLASQ2.
*
*  Further Details
...
*     .. External Functions ..
      LOGICAL            XLSAME
      DOUBLE PRECISION            XDLAMCH
      EXTERNAL           XDLAMCH, XLSAME

*     ..
*     .. External Subroutines ..
      EXTERNAL           XDCOPY, XDLARNV, XDLARRA, XDLARRB, XDLARRC,
     $                   XDLARRD, XDLASQ2
*     ..
*     .. Intrinsic Functions ..
...
*     Decode RANGE
*
      IF( XLSAME( RANGE, 'A' ) ) THEN
         IRANGE = ALLRNG
      ELSE IF( XLSAME( RANGE, 'V' ) ) THEN
         IRANGE = VALRNG
      ELSE IF( XLSAME( RANGE, 'I' ) ) THEN
         IRANGE = INDRNG
      END IF
...
*     Compute splitting points
      CALL XDLARRA( N, D, E, E2, SPLTOL, SPDIAM,
     $                    NSPLIT, ISPLIT, IINFO )
...
*        Set interval [VL,VU] that contains all eigenvalues
         ...
         VU = GU
      ELSE
*        We call XDLARRD to find crude approximations to the eigenvalues
*        in the desired range. In case IRANGE = INDRNG, we also obtain the
*        interval (VL,VU] that contains all the wanted eigenvalues.
*        An interval [LEFT,RIGHT] has converged if
*        RIGHT-LEFT.LT.RTOL*MAX(ABS(LEFT),ABS(RIGHT))
*        XDLARRD needs a WORK of size 4*N, IWORK of size 3*N
         CALL XDLARRD( RANGE, 'B', N, VL, VU, IL, IU, GERS,
...
*           Find approximations to the extremal eigenvalues of the block
            CALL XDLARRK( IN, 1, GL, GU, D(IBEGIN),
     ...
     $               - HNDRD * EPS* ABS(TMP - TMP1))
            ...
            CALL XDLARRK( IN, IN, GL, GU, D(IBEGIN),
     $               E2(IBEGIN), PIVMIN, RTL, TMP, TMP1, IINFO )
            IF( IINFO.NE.0 ) THEN
               INFO = -1
               RETURN
...
*        the eigenvalue approximations at the end of XDLARRE or bisection.
...
*           Define 1/4 and 3/4 points of the spectrum
         ...
         ELSE
*           XDLARRD has computed IBLOCK and INDEXW for each eigenvalue
*           approximation.
*           choose sigma
            IF( USEDQD ) THEN
               S1 = ISLEFT + FOURTH * SPDIAM
...
*        Compute the negcount at the 1/4 and 3/4 points
         IF(MB.GT.1) THEN
            CALL XDLARRC( 'T', IN, S1, S2, D(IBEGIN),
...
*        Store the shift.
         E( IEND ) = SIGMA
*        Store D and L.
         CALL XDCOPY( IN, WORK, 1, D( IBEGIN ), 1 )
         CALL XDCOPY( IN-1, WORK( IN+1 ), 1, E( IBEGIN ), 1 )
...
*
 ...
 122        CONTINUE
            ...
            CALL XDLARNV(2, ISEED, 2*IN-1, WORK(1))
            DO 125 I = 1,IN-1
...
*        Compute the required eigenvalues of L D L' by bisection or dqds
         IF ( .NOT.USEDQD ) THEN
*           If XDLARRD has been used, shift the eigenvalue approximations
...
*           UNshifted eigenvalue approximation.
 ...
 134        CONTINUE
*           call XDLARRB to reduce eigenvalue error of the approximations
*           from XDLARRD
            DO 135 I = IBEGIN, IEND-1
               WORK( I ) = D( I ) * E( I )**2
 135        CONTINUE
*           use bisection to find EV from INDL to INDU
            CALL XDLARRB(IN, D(IBEGIN), WORK(IBEGIN),
     ...
     $                  IN, IINFO )
            IF( IINFO .NE. 0 ) THEN
               ...
               RETURN
            END IF
*           XDLARRB computes all gaps correctly except for the last one
...
*           Set RTOL to the order of the tolerance used in XDLASQ2
...
*              Test that all eigenvalues are positive as expected
 ...
 160           CONTINUE
            ...
            DO 165 I = M - MB + 1, M
*              the value of RTOL below should be the tolerance in XDLASQ2
               WERR( I ) = RTOL * ABS( W(I) )
 165        CONTINUE
            DO 166 I = M - MB + 1, M - 1
