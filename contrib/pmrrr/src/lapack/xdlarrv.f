      SUBROUTINE XDLARRV( N, VL, VU, D, L, PIVMIN,
...
*  =======
*
*  XDLARRV computes the eigenvectors of the tridiagonal matrix
...
*          = 0:  successful exit
*
*          > 0:  A problem occured in XDLARRV.
*          < 0:  One of the called subroutines signaled an internal problem.
*                Needs inspection of the corresponding parameter IINFO
*                for further information.
*
*          =-1:  Problem in XDLARRB when refining a child's eigenvalues.
*          =-2:  Problem in XDLARRF when computing the RRR of a child.
...
*          =-3:  Problem in XDLARRB when refining a single eigenvalue
...
*     .. External Subroutines ..
      EXTERNAL           XDCOPY, XDLAR1V, XDLARRB, XDLARRF, XDLASET,
     $                   XDSCAL
*     ..
*     .. Intrinsic Functions ..
...
*     The width of the part of Z that is used
      ...
      CALL XDLASET( 'Full', N, ZUSEDW, ZERO, ZERO,
     $                    Z(1,ZUSEDL), LDZ )
...
*                       Get representation from the right end of Z array
                     ...
                     ENDIF
                  ENDIF
                  CALL XDCOPY( IN, Z( IBEGIN, J ), 1, D( IBEGIN ), 1 )
                  CALL XDCOPY( IN-1, Z( IBEGIN, J+1 ), 1, L( IBEGIN ),
     $               1 )
                  SIGMA = Z( IEND, J+1 )

*                 Set the corresponding entries in Z to zero
                  CALL XDLASET( 'Full', IN, 2, ZERO, ZERO,
     $                         Z( IBEGIN, J), LDZ )
               END IF
...
*                 WBEGIN-1+OLDLST are correctly computed in XDLARRB.
...
*
C                    XDLARRF needs LWORK = 2*N
                     CALL XDLARRF( IN, D( IBEGIN ), L( IBEGIN ),
     $                         WORK(INDLD+IBEGIN-1),
     ...
     $                         WORK( INDWRK ), IINFO )
                     IF( IINFO.EQ.0 ) THEN
*                       a new RRR for the cluster was found by XDLARRF
...
*                    Given LAMBDA, compute the eigenvector.
                     CALL XDLAR1V( IN, 1, IN, LAMBDA, D( IBEGIN ),
...
*                    Ensure vector is ok if support in the RQI has changed
 ...
 123                    CONTINUE
                     ENDIF
                     CALL XDSCAL( ZTO-ZFROM+1, NRMINV,
     $                       Z( ZFROM, WINDEX ), 1 )
 125                 CONTINUE
*                    Update W
