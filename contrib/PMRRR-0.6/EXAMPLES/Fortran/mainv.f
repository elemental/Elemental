...
*     Matrix Z must be bigger than n*nz doubles, this can be check
*     by calling 'pmrrr', where the value 'nz' is set by the following
      CALL PMRRR('C', 'V', N, D, E, VL, VU, IL, IU, TRYRAC, 
...
*     If Z big enough, call routine to compute eigenpairs
      CALL PMRRR('V', 'V', N, D, E, VL, VU, IL, IU, TRYRAC, 
     $           MPI_COMM_WORLD, NZ, OFFSET, W, Z, LDZ, ZSUPP, IERR)
      IF (IERR .NE. 0) THEN
