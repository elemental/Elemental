...
def TriangEigenvecs(U):
  if type(U) is Matrix:
    X = Matrix(U.tag)
    args = [U.obj,X.obj]
    if   U.tag == sTag: lib.ElTriangEigenvecs_s(*args)
    elif U.tag == dTag: lib.ElTriangEigenvecs_d(*args)
    elif U.tag == cTag: lib.ElTriangEigenvecs_c(*args)
    elif U.tag == zTag: lib.ElTriangEigenvecs_z(*args)
    else: DataExcept()
    return X
  elif type(U) is DistMatrix:
    X = DistMatrix(U.tag,MC,MR,U.Grid())
    args = [U.obj,X.obj]
    if   U.tag == sTag: lib.ElTriangEigenvecsDist_s(*args)
    elif U.tag == dTag: lib.ElTriangEigenvecsDist_d(*args)
    elif U.tag == cTag: lib.ElTriangEigenvecsDist_c(*args)
    elif U.tag == zTag: lib.ElTriangEigenvecsDist_z(*args)
    else: DataExcept()
    return X
  else: TypeExcept()
