...
# If USER_MATH_LIBS exists, see if it supports ScaLAPACK
if(USER_MATH_LIBS)
  ...
  set(CMAKE_REQUIRED_LIBRARIES ${USER_MATH_LIBS} ${MPI_C_LIBRARIES})
  ...
  if(EL_HAVE_DTRSM OR EL_HAVE_DTRSM_POST)
    ...
    message(WARNING "Could not find BLAS in ${USER_MATH_LIBS}")
  endif()
  ...
  if(EL_HAVE_DSYTRD OR EL_HAVE_DSYTRD_POST)
    ...
    message(WARNING "Could not find LAPACK in ${USER_MATH_LIBS}")
  endif()
  if(EL_SCALAPACK_SUFFIX)
    ...
    elseif(HAVE_BLAS AND HAVE_LAPACK)
      set(USER_MATH_LIBS_HAS_SCALAPACK TRUE)
  ...
  if(USER_MATH_LIBS_HAS_SCALAPACK)
    ...
    if(NOT EL_HAVE_PDLAQR0 OR NOT EL_HAVE_PDLAQR1)
      message(STATUS "USER_MATH_LIBS=${USER_MATH_LIBS} supports some of ScaLAPACK, but not PDLAQR{0,1}.")
      set(USER_MATH_LIBS_HAS_SCALAPACK FALSE)
    endif()
  endif()
...
if(NOT USER_MATH_LIBS_HAS_SCALAPACK AND NOT EL_BUILD_SCALAPACK)
  ...
  if(ScaLAPACK)
    ...
    if(EXISTS "${ScaLAPACK_DIR}/${CMAKE_SHARED_LIBRARY_PREFIX}blacs${CMAKE_SHARED_LIBRARY_SUFFIX}" OR 
      ...
      set(CMAKE_REQUIRED_INCLUDES ${MPI_C_INCLUDE_PATH})
      set(CMAKE_REQUIRED_LIBRARIES 
        ${ScaLAPACK} ${USER_MATH_LIBS} ${MPI_C_LIBRARIES})
      El_check_function_exists(dtrsm   EL_HAVE_DTRSM)
      El_check_function_exists(dtrsm_  EL_HAVE_DTRSM_POST)
      if(EL_HAVE_DTRSM OR EL_HAVE_DTRSM_POST)
        set(HAVE_BLAS TRUE)
      else()
        message(WARNING "Could not link BLAS")
      endif()
      El_check_function_exists(dsytrd  EL_HAVE_DSYTRD)
      El_check_function_exists(dsytrd_ EL_HAVE_DSYTRD_POST)
      if(EL_HAVE_DSYTRD OR EL_HAVE_DSYTRD_POST)
        ...
        message(WARNING "Could not link LAPACK")
      endif()
      if(EL_SCALAPACK_SUFFIX)
        El_check_function_exists(pdsyngst${EL_SCALAPACK_SUFFIX} EL_HAVE_PDSYNGST)
        El_check_function_exists(Csys2blacs_handle EL_HAVE_CSYS2BLACS)
...
elseif(EL_HAVE_F90_INTERFACE AND EL_HAVE_MPI_FORTRAN)
  ...
  if(USER_MATH_LIBS)
    set(LAPACK_COMMAND -D LAPACK_LIBRARIES=${USER_MATH_LIBS})
  elseif(APPLE)
  ...
  if(USER_MATH_LIBS)
    if(WIN32)
      set(SCALAPACK_LIBS ${SCALAPACK_LIB} ${SCALAPACKF_LIB} ${USER_MATH_LIBS})
    else()
      set(SCALAPACK_LIBS ${SCALAPACK_LIB} ${USER_MATH_LIBS})
    endif()
