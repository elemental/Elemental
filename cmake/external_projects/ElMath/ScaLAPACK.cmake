...
elseif(EL_HAVE_F90_INTERFACE AND EL_HAVE_MPI_FORTRAN)
  ...
  elseif(APPLE)
    ...
    elseif(EL_HAVE_MKL AND NOT EL_DISABLE_MKL)
      set(EL_USING_MKL TRUE)
    elseif(EL_HAVE_ACCELERATE)
      set(EL_USING_ACCELERATE TRUE)
    elseif(EL_HAVE_VECLIB)
      set(EL_USING_VECLIB TRUE)
    elseif(NOT DISABLE_OPENBLAS)
      set(EL_BUILT_OPENBLAS TRUE)
    elseif(NOT DISABLE_BLIS_LAPACK)
      set(EL_BUILT_BLIS_LAPACK TRUE)
    else()
      set(EL_USING_REF_LAPACK TRUE)
    endif()
  elseif(NOT MSVC)
    if(EL_HAVE_MKL AND NOT EL_DISABLE_MKL AND 
       NOT EL_PREFER_BLIS_LAPACK AND NOT EL_PREFER_OPENBLAS)
      set(EL_USING_MKL TRUE)
    elseif(NOT DISABLE_OPENBLAS)
      set(EL_BUILT_OPENBLAS TRUE)
    elseif(NOT DISABLE_BLIS_LAPACK)
    ...
    if(EL_HAVE_MKL AND NOT EL_DISABLE_MKL)
      set(EL_USING_MKL TRUE)
    else()
      set(EL_USING_REF_LAPACK TRUE)
    endif()
  ...
  elseif(EL_USING_MKL)
    set(SCALAPACK_LIBS ${SCALAPACK_BASE} ${MKL_LIBS})
    set(SCALAPACK_LIBS_AT_CONFIG ${MKL_LIBS})
  elseif(EL_USING_VECLIB)
    set(SCALAPACK_LIBS ${SCALAPACK_BASE} ${VECLIB_LIBS})
    set(SCALAPACK_LIBS_AT_CONFIG ${VECLIB_LIBS})
  elseif(EL_USING_ACCELERATE)
    set(SCALAPACK_LIBS ${SCALAPACK_BASE} ${ACCELERATE_LIBS})
    set(SCALAPACK_LIBS_AT_CONFIG ${ACCELERATE_LIBS})
  elseif(EL_USING_REF_LAPACK)
    find_package(LAPACK)
    if(LAPACK_FOUND)
      set(SCALAPACK_LIBS ${SCALAPACK_BASE} ${LAPACK_LIBRARIES})
      set(SCALAPACK_LIBS_AT_CONFIG ${LAPACK_LIBRARIES})
    else()
      message(FATAL_ERROR "Could not find LAPACK")
    endif()
  else()
    message(FATAL_ERROR "Fallthrough for BLAS/LAPACK")
  endif()
